...
Usage:
    python src/export_excel.py 1887               # all entries
    python src/export_excel.py 1887 --women-only  # only women (is_woman=Y)
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python src/export_excel.py YEAR [--women-only]")
        sys.exit(1)
    year = int(sys.argv[1])
