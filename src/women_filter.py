...
Usage:
    python src/women_filter.py 1887
"""
...
from index_reader import read_feminine_first_names
...
# Must match the column order defined in prompts.py EXACTLY
# ─────────────────────────────────────────────
...
MASTER_COLUMNS = [
    ...
    # person fields
    "profession_section",
    "full_name_raw", "civil_status", "diploma_year",
...
def _detect_woman(row: Dict[str, str], fem_names: Set[str]) -> str:
    ...
    # 1. Check the dedicated civil_status field
    cs = row.get("civil_status", "").strip().lower()
    if cs:
...
def merge_and_annotate(year: int, settings: Settings) -> Path:
    ...
    # ── Forward-fill geographic context within each list_type section ──────
    # If Gemini left a field blank (header was on a previous page), propagate
    # the last seen non-empty value within the same list_type block.
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python src/women_filter.py YEAR")
        sys.exit(1)
    year = int(sys.argv[1])
    settings = Settings()
